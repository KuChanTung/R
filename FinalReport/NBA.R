...
#條件推論樹
ct <- ctree(Stats ~ ., data = NBA)
plot(ct, main = "Conditional Inference Tree")
table(NBA$Stats, predict(ct))
...
#使用訓練樣本進行建模與評估辨識正確率88.82%
NBA.rf <-randomForest(Stats~.,data=NBA.train,importance=TRUE,proximity=TRUE,do.trace = 100)
plot(NBA.rf)
print(NBA.rf)
...
#找出最佳mtry參數
n=ncol(NBA)-1 #取23-1=22個屬性，即扣掉要預測的狀態
model_err_rate=1
for(i in 1:n)
{
  result=randomForest(Stats~.,data=NBA,mtry=i,importance=TRUE,ntree=500,do.trace = 100)
...
#重新設定mtry參數，並使用訓練樣本重新建模與評估辨識正確率88.82%
NBA.rf.Opt <-randomForest(Stats~.,data=NBA.train,mtry=mtry_value,importance=TRUE,proximity=TRUE,ntree=500,do.trace = 100)
print(NBA.rf.Opt)
#畫圖判別ntree參數設定
plot(NBA.rf.Opt)
#重新檢視重要變數並畫圖
...
#畫出群集分析圖
MDSplot(NBA.rf.Opt,NBA$Stats,palette=rep(1,5), pch=as.numeric(NBA$Stats))
